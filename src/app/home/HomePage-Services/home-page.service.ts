...
export class HomePageService {
  ...
  getProducts():Observable<IProduct[]>{
    let tempVar = this.http.get<IProduct[]>('https://quickcartvin.azurewebsites.net/api/home/getproducts')
    console.log(tempVar)
    return tempVar
  }
  ...
  ValidateUser(userEmailID:string, userPassword:string, type:string):Observable<number>
  {
    ...
    let result=this.http.post<number>('https://functionappdemotlt.azurewebsites.net/api/LoginFunction?code=I6PdPrYaqovYNc-rzr5x1Fc24aPPYpqs5jYuz3uounncAzFusXopjA==',user)
    return result
